...
function handleImport() {
  ...
  reader.onload = function(e) {
    try {
      ...
      renderizarProfissionais() // Atualiza a lista de profissionais
      alert(`Importação concluída! ${dados.length} profissionais tentaram ser importados.`)
...
function atualizarSemana() {
  ...
  datasVisiveis.forEach((data, index) => {
    ...
    if (dataElement) {
      ...
      // Ajuste o nome do dia para bater com o HTML (Segunda, Terça, etc.)
      const diaNome = diasSemana[index];
      const headerElement = document.getElementById(`header-dia-${index + 1}`);
      if(headerElement) {
          headerElement.querySelector('.dia-nome').textContent = diaNome;
      }
...
function atualizarSlotsComDatas() {
  // Ajuste o seletor para pegar todos os slots e seus containers
  const slots = document.querySelectorAll(".slot-horario")
  
  slots.forEach((slot) => {
    // Busca o dia da semana pelo atributo data-dia (ex: "Segunda")
    const diaSemanaNome = slot.getAttribute("data-dia")
    const diaIndex = diasSemana.indexOf(diaSemanaNome) // 0 para Segunda, 1 para Terça, etc.
...
function obterHorariosProximos(horarioBase, periodo, quantidade = 1) { // Ajuste para gerar apenas 1 por vez
  ...
  return horarios.slice(0, quantidade) // Garante a quantidade exata
}
...
function gerarVinculos(pacienteId) {
  ...
  // Mapeamento de dias da semana para índice (0=Domingo, 1=Segunda)
  const diaMap = {
      "Segunda": 1, "Terça": 2, "Quarta": 3, "Quinta": 4, "Sexta": 5
  };
  
  // Determinar dias disponíveis baseado nas preferências (usando nomes completos)
  let diasDisponiveisNomes = paciente.diasPreferenciais.length > 0 ? 
    paciente.diasPreferenciais : diasSemana

  // Gerar até 4 opções de escalas
  for (let opcao = 0; opcao < 4; opcao++) {
    const escalaOpcao = []
    let profissionalUsadoNestaOpcao = new Set();
    let diasAtendimentoUsados = new Set();

    // Para cada especialidade do paciente
    paciente.especialidades.forEach((especialidade, espIndex) => {
      
      // 1. Tenta buscar profissionais disponíveis
      let profissionaisDisponiveis = profissionais.filter((p) => 
          p.especialidade === especialidade && 
          p.disponivel &&
          !profissionalUsadoNestaOpcao.has(p.id)
      );

      // CORREÇÃO: SE NÃO ENCONTRAR PROFISSIONAIS, CRIA UM NOVO PLACEHOLDER
      if (profissionaisDisponiveis.length === 0) {
          console.warn(`Nenhum profissional encontrado para ${especialidade}. Inserindo placeholder.`);
          ...
          const novoProfissional = {
              id: novoId,
              nome: `Profissional Novo (${especialidade})`, // Nome genérico
              especialidade: especialidade,
              disponivel: true 
          };

          profissionais.push(novoProfissional); // Insere na lista global
          salvarDados(); // Salva os dados atualizados
          
          // Re-filtra (agora ele encontrará o profissional recém-inserido)
          profissionaisDisponiveis = [novoProfissional];
      }

      // Se ainda for zero após a tentativa de inserção (o que é improvável), retorna.
      if (profissionaisDisponiveis.length === 0) return;
      
      // Escolhe um profissional de forma rotativa
      const profissionalEscolhido = profissionaisDisponiveis[opcao % profissionaisDisponiveis.length];
      profissionalUsadoNestaOpcao.add(profissionalEscolhido.id);
      
      // Escolher um dia baseado nas preferências
      const diaEscolhidoNome = diasDisponiveisNomes[espIndex % diasDisponiveisNomes.length]
      
      // Calcular a data real do atendimento (Próxima ocorrência do dia escolhido a partir da data de início)
      const diaDesejadoIndex = diaMap[diaEscolhidoNome];
      const dataAtendimento = new Date(dataInicioPaciente);
      let diaAtualIndex = dataAtendimento.getDay();
      
      let diasParaAdicionar = (diaDesejadoIndex - diaAtualIndex + 7) % 7;
      
      // Se o dia da semana já tiver sido usado, pula para o próximo dia disponível/útil
      let tentativa = 0;
      let dataTentativa = new Date(dataAtendimento.getTime());
      dataTentativa.setDate(dataAtendimento.getDate() + diasParaAdicionar);
      
      while (diasAtendimentoUsados.has(dataTentativa.toDateString()) && tentativa < 7) {
          diasParaAdicionar++; // Pula para o próximo dia
          dataTentativa = new Date(dataAtendimento.getTime());
          dataTentativa.setDate(dataAtendimento.getDate() + diasParaAdicionar);
          tentativa++;
      }
      
      dataAtendimento.setDate(dataAtendimento.getDate() + diasParaAdicionar);
      diasAtendimentoUsados.add(dataAtendimento.toDateString());
      ...
      if (paciente.periodoPreferencia === "manha") {
        ...
        periodo = espIndex % 2 === 0 ? "manha" : "tarde"
      }

      // Escolher um horário base e gerar o horário próximo (1 horário por especialidade)
      const horariosDisponiveis = periodo === "manha" ? horariosManha : horariosTarde
      const horarioBase = horariosDisponiveis[Math.floor(Math.random() * horariosDisponiveis.length)]
      const horariosProximos = obterHorariosProximos(horarioBase, periodo, 1) // Garantido 1 horário
      
      horariosProximos.forEach((horario) => {
        
        escalaOpcao.push({
          dia: diaEscolhidoNome,
          data: dataAtendimento.toISOString().split("T")[0],
          horario: horario,
  ...
  if(escalasOpcoes.length === 0){
      alert("Não foi possível gerar opções de escala! Verifique se há especialidades selecionadas ou se a lógica de dias/horários está muito restritiva.");
      return;
  }
...
function renderizarEscalasGeradas() {
  ...
  escalasGeradas.forEach((escala) => {
    ...
    escala.opcoes.forEach((opcao, index) => {
      ...
      opcaoDiv.innerHTML = `
        <h3>Opção ${index + 1}</h3>
        <div class="escala-itens-wrapper">${itensHtml}</div>
...
function renderizarVinculos() {
  ...
  vinculosSemana.forEach((vinculo) => {
    ...
    // O seletor usa o dia da semana ('Segunda', 'Terça') e o horário ('HH:MM')
